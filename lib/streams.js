    ...
    return track(
        new Duplex({
            ...
            write(chunk, encoding, callback) {
                ...
                try {
                    chunk = instruments.play(chunk)
                } catch (e) {
                    process.stderr.write(chalk.red(`Cannot parse note "${chunk.input}"\n`))
                    return callback()// return callback(new Error(`Cannot parse note "${chunk.input}"`))
                }
                callback(null, chunk)
                this.push(chunk)
            }
...
exports.on = instrument => {
    return new Transform({
        ...
        transform(chunk, encoding, callback) {
            ...
            if (!instrument) {
                // select and persist random instrument
                instrument = shuffle(instruments.all)[0]
            }
