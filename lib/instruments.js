        ...
        return {
            play(note) {
                ...
                function pathToAudio(note) {
                    return path.join(pathToSoundfonts, `${instrumentName}-mp3`, `${note.toString().toUpperCase()}.mp3`)
                }
                ...
                console.log(`${instrumentName}: ${noteThatExists.note.fq() < 130 ? '𝄢' : '𝄞'} ♩ ${teoria.note(note).toString()}`)
            }
        }
    }
}
